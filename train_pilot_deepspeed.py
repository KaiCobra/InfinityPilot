...
def build_model_optimizer_nonused(args, vae_ckpt):
    ...
    param_name_lookup = {id(param): name for name, param in zip(names, paras)}

    def _split_group(group):
        base_params, block_params, fusion_params = [], [], []
        for param in group['params']:
            pname = param_name_lookup.get(id(param))
            if pname is None:
                base_params.append(param)
                continue
            if 'car_blocks' in pname:
                block_params.append(param)
            elif any(key in pname for key in ('car_fusion_', 'car_skip_', 'car_control_proj')):
    ...
    for group in para_groups:
        base_lr_scale = group.get('lr_sc', 1.0)
        base_params, block_params, fusion_params = _split_group(group)

        if base_params:
            new_group = dict(group)
            new_group['params'] = base_params
            new_group['lr_sc'] = base_lr_scale * args.car_lr_scale
            expanded_groups.append(new_group)

        if block_params:
            new_group = dict(group)
            new_group['params'] = block_params
            new_group['lr_sc'] = base_lr_scale * args.car_lr_scale * args.car_block_lr_scale
            expanded_groups.append(new_group)

        if fusion_params:
            new_group = dict(group)
            new_group['params'] = fusion_params
            new_group['lr_sc'] = base_lr_scale * args.car_lr_scale * args.car_fusion_lr_scale
            expanded_groups.append(new_group)
