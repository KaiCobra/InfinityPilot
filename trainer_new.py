...
class InfinityTrainer(object):
    ...
    inp_B3HW: FTen, condition_B3HW: FTen, text_cond_tuple: Union[ITen, FTen], args: arg_util.Args,
        ...
        with self.gpt_opt.amp_ctx:
            ...
            self.set_task_id(1)
            logits_BLV_1 = self.gpt(text_cond_tuple, x_BLC_wo_prefix, scale_schedule=scale_schedule[:training_scales]) # [bs, 1*1+...+64*64, vocab_size or log2(vocab_size)*2]
            ...
            if args.use_bit_label: # default 1
                ...
                loss_0 = self.train_loss(logits_BLV_0.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), gt_BL)
                tmp_bs, tmp_seq_len, tmp_channel = logits_BLV_1.shape
                loss_1 = self.train_loss(logits_BLV_1.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), gt_BL_condition)
                if args.bitloss_type == 'mean': # default mean
                    loss_0 = loss_0.mean(dim=-1)
                    loss_1 = loss_1.mean(dim=-1)
                elif args.bitloss_type == 'sum':
                    loss_0 = loss_0.sum(dim=-1)
                    loss_1 = loss_1.sum(dim=-1)
                else:
                    raise NotImplementedError(f'{args.bitloss_type=}')
            else:
                loss_0 = self.train_loss(logits_BLV_0.reshape(-1, V), gt_BL.reshape(-1)).reshape(B, -1)
                loss_1 = self.train_loss(logits_BLV_condition.reshape(-1, V), gt_BL_condition.reshape(-1)).reshape(B, -1)

            if self.reweight_loss_by_scale: # default 1
                lw = []
                ...
                lw = torch.tensor(lw, device=loss_0.device)[None, ...]
                lw = lw / lw.sum()
            else:
                lw = 1. / self.seq_len
            loss_0 = loss_0.mul(lw).sum(dim=-1).mean()
            loss_1 = loss_1.mul(lw).sum(dim=-1).mean()
            loss = loss_0 + 0.7*loss_1
