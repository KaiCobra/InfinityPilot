...
data_root = "/home/avlab/SceneTxtVAR/selected_images"
data_img_pth = osp.join(data_root, "images")

for i, img in enumerate(sorted(os.listdir(data_img_pth))):
    img_pth = osp.join(data_img_pth, img)
    metadata_pth = img_pth.replace('images', 'metadatas')
    metadata_pth = metadata_pth.replace('.png', '.json')
    metadata_pth = metadata_pth.replace('selected_metadatas', 'selected_images')
    ...
    prompt = "Render the text \"" + "\" and \"".join(matches) + "\" in the following image: " + metadata['caption']
    print(prompt)
    # continue
    ...
    args.save_file = './generated_3d3/' + img
