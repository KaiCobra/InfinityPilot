...
def build_model_optimizer(args, vae_ckpt):
    ...
    '''
    block_chunks.0.module.0.sa.mat_qkv_tasks.[0,1].weight
    '''
    ...
    if args.rush_resume:
        ...
        def drop_unfit_weights(state_dict):
            ...
            qkv_keys = [k for k in state_dict.keys() if 'sa.mat_qkv' in k]
            # print("\nFound QKV keys to process:", qkv_keys)
            for key in qkv_keys:
                # 創建新的權重名稱
                new_key1 = key.replace('sa.mat_qkv', 'sa.mat_qkv_tasks.0')
                new_key2 = key.replace('sa.mat_qkv', 'sa.mat_qkv_tasks.1')
                
                # 複製權重
                state_dict[new_key1] = state_dict[key].clone()
                state_dict[new_key2] = state_dict[key].clone()
            ...
            ffns_keys = [k for k in state_dict.keys() if 'ffn' in k]
            for key in ffns_keys:
                # 分離出前綴和後綴
                prefix, ffn_part = key.rsplit('ffn.', 1)
                
                # 創建兩個新的權重名稱
                new_key1 = f"{prefix}ffns.0.{ffn_part}"
                new_key2 = f"{prefix}ffns.1.{ffn_part}"
            ...
            # 處理 q_bias 和 v_bias
            bias_keys = [k for k in state_dict.keys() if 'sa.q_bias' in k or 'sa.v_bias' in k]
            for key in bias_keys:
                # 分離出前綴和後綴
                prefix, bias_part = key.rsplit('.', 1)
                
                # 創建兩個新的權重名稱
                new_key1 = f"{prefix}.{bias_part}_tasks.0"
                new_key2 = f"{prefix}.{bias_part}_tasks.1"
        ...
        if args.use_fsdp_model_ema: # false
