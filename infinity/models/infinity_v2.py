...
from infinity.models.basic_v2 import flash_attn_func, flash_fused_op_installed, AdaLNBeforeHead, CrossAttnBlock, SelfAttnBlock, CrossAttention, FastRMSNorm, precompute_rope2d_freqs_grid
    ...
    def __init__(
        ...
        for block_idx in range(depth):
            block = (CrossAttnBlock if self.t2i else SelfAttnBlock)(
                ...
                task_id=task_id if self.t2i else None,
            )
        ...
        print(
            f'\n[constructor]  ==== customized_flash_attn={self.customized_flash_attn} (using_flash={sum((b.sa.using_flash if self.t2i else b.attn.using_flash) for b in self.unregistered_blocks)}/{self.depth}), fused_mlp={fused_mlp} (fused_mlp={sum(b.ffns[self.task_id].fused_mlp_func is not None for b in self.unregistered_blocks)}/{self.depth}) ==== \n'
    ...
    def forward(self, label_B_or_BLT: Union[torch.LongTensor, Tuple[torch.FloatTensor, torch.IntTensor, int]], x_BLC_wo_prefix: torch.Tensor, scale_schedule: List[Tuple[int]],
        ...
        **kwargs,
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if self.customized_flash_attn: #false
        ...
        if self.num_block_chunks == 1: #false
    ...
    def special_init(
        ...
        for block_idx, sab in enumerate(self.unregistered_blocks):
            ...
            if scale_proj == 1:
                ...
                sab.ffns[self.task_id].fc2.weight.data.mul_(scale)
            # if sab.using_swiglu:
