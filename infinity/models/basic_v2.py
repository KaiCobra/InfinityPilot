...
try:
    ...
    def rms_norm_impl(x, weight, epsilon):
        # Make sure we're working with a float tensor
        x_float = x.float() if not x.is_floating_point() else x
        
        # Calculate variance without inplace operations
        variance = torch.mean(torch.square(x_float), dim=-1, keepdim=True)
        
        # Calculate inverse standard deviation safely
        inv_std = torch.rsqrt(variance + epsilon)
        
        # Apply normalization and scaling
        normalized = x_float * inv_std
...
class FastRMSNorm(nn.Module):
    ...
    def forward(self, x):
        src_type = x.dtype
        return rms_norm_impl(x, self.weight, epsilon=self.eps)
