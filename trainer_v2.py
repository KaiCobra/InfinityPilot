...
from infinity.models import infinity_v2 as Infinity
...
class InfinityTrainer(object):
    ...
    def set_task_id(self, new_id: int):
        """
        针对 self.gpt（DDP/FSDP 包装后的模型）切换 task_id，
        先 unwrap 然后给所有带有 task_id 属性的子模块赋值。
        """
        # 1. 拆 DDP / FSDP 的壳
        m = self.gpt
        # 如果是 DDP，就拿 .module
        if hasattr(m, "module"):
            m = m.module
        # 如果又被 torch.compile 包了一层，就拿 _orig_mod
        ...
        # 3. 遍历所有子模块，凡是有 task_id 属性的一律改掉
        for sub in m.modules():
            if hasattr(sub, "task_id"):
                sub.task_id = new_id

    def train_step(
    self, ep: int, it: int, g_it: int, stepping: bool, clip_decay_ratio: float, 
    metric_lg: misc.MetricLogger, logging_params: bool,
    inp_B3HW: FTen, condition_B3HW: FTen, text_cond_tuple: Union[ITen, FTen], args: arg_util.Args,
    ) -> Tuple[torch.Tensor, Optional[float]]:
        ...
        with self.gpt_opt.amp_ctx:
            with torch.amp.autocast('cuda', enabled=False):
                with torch.no_grad():
                    ...
                    raw_features_condition, _, _ = self.vae_local.encode_for_raw_features(condition_B3HW, scale_schedule=vae_scale_schedule)
            
            x_BLC_wo_prefix, gt_ms_idx_Bl = self.bitwise_self_correction.flip_requant(vae_scale_schedule, inp_B3HW, raw_features, device)
            x_BLC_wo_prefix_condition, gt_ms_idx_Bl_condition = self.bitwise_self_correction.flip_requant(vae_scale_schedule, condition_B3HW, raw_features_condition, device)
            ...
            x_BLC_wo_prefix_condition = x_BLC_wo_prefix_condition[:, :(training_seq_len-np.array(scale_schedule[0]).prod()), :] # cut scales according to training_scales   

            self.gpt_wo_ddp.forward  
            self.set_task_id(0)
            logits_BLV_0 = self.gpt(text_cond_tuple, x_BLC_wo_prefix, scale_schedule=scale_schedule[:training_scales]) # [bs, 1*1+...+64*64, vocab_size or log2(vocab_size)*2]
            self.set_task_id(1)
            logits_BLV_1 = self.gpt(text_cond_tuple, x_BLC_wo_prefix_condition, scale_schedule=scale_schedule[:training_scales]) # [bs, 1*1+...+64*64, vocab_size or log2(vocab_size)*2]
            self.batch_size, self.seq_len = logits_BLV_0.shape[:2]
            ...
            self.seq_len_each_condition = [idx_Bl.shape[1] for idx_Bl in gt_ms_idx_Bl_condition]            
            gt_BL_condition = torch.cat(gt_ms_idx_Bl_condition, dim=1)[:,:training_seq_len].contiguous().type(torch.long) # [bs, 1*1+...+64*64, 16] or [bs, 1*1+...+64*64]
            
            if args.use_bit_label:
                tmp_bs, tmp_seq_len, tmp_channel = logits_BLV_0.shape
                loss = self.train_loss(logits_BLV_0.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), gt_BL)
                loss_condition = self.train_loss(logits_BLV_1.reshape(tmp_bs, tmp_seq_len, -1, 2).permute(0,3,1,2), gt_BL_condition)
                if args.bitloss_type == 'mean':
                    loss = loss.mean(dim=-1)
                    loss_condition = loss_condition.mean(dim=-1)
                elif args.bitloss_type == 'sum':
                    loss = loss.sum(dim=-1)
                    loss_condition = loss_condition.sum(dim=-1)
                else:
                    raise NotImplementedError(f'{args.bitloss_type=}')
            else:
                loss = self.train_loss(logits_BLV_0.reshape(-1, V), gt_BL.reshape(-1)).reshape(B, -1)
                loss_condition = self.train_loss(logits_BLV_1.reshape(-1, V), gt_BL_condition.reshape(-1)).reshape(B, -1)
            ...
            loss_condition = loss_condition.mul(lw).sum(dim=-1).mean()
        ...
        if metric_lg.log_every_iter or it == 0 or it in metric_lg.log_iters:
            B, seq_len = logits_BLV_0.shape[:2]
            if args.use_bit_label:
                res_loss = self.train_loss(logits_BLV_0.reshape(B, seq_len, -1, 2).permute(0,3,1,2), gt_BL).mean(dim=-1).mean(0)
                bitwise_acc = (logits_BLV_0.reshape(B, seq_len, -1, 2).argmax(dim=-1) == gt_BL).float() # shape: [bs, seq_len, codebook_dim]
            else:
                res_loss = self.train_loss(logits_BLV_0.reshape(-1, V), gt_BL.reshape(-1)).reshape(B, -1).mean(0)
                pred_BL = logits_BLV_0.argmax(dim=-1)
